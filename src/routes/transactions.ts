...
const balanceMask = (balanceMinor: number) => {
  const masked = Math.floor(balanceMinor / 100).toString();
  const keep = masked.slice(-2);
  return `****${keep}`;
};

transactionsRouter.get('/balance', requireAuth, async (req: AuthedRequest, res) => {
  const uid = req.user!.uid;
  const doc = await firestore.collection('accounts').doc(uid).get();
  const data = doc.exists ? doc.data() as any : { balanceMinor: 0 };
  const masked = balanceMask(data.balanceMinor || 0);
  res.json({ balance: masked, masked: true });
});

transactionsRouter.post('/balance/unmask', requireAuth, async (req: AuthedRequest, res) => {
  ...
  const valid = tokenDoc.exists && (tokenDoc.data() as any).uid === uid && (tokenDoc.data() as any).expiresAt > Date.now();
  if (!valid) return res.status(403).json({ error: 'step_up_required' });
  const doc = await firestore.collection('accounts').doc(uid).get();
  const data = doc.exists ? (doc.data() as any) : { balanceMinor: 0 };
...
transactionsRouter.post('/export', requireAuth, async (req: AuthedRequest, res) => {

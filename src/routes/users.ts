...
authRouter.post('/sms/start', async (req, res) => {
  if (!client || !env.TWILIO_VERIFY_SERVICE_SID) return res.status(500).json({ error: 'Verify not configured' });
...
authRouter.post('/stepup/verify', requireAuth, async (req: AuthedRequest, res) => {
  if (!client || !env.TWILIO_VERIFY_SERVICE_SID) return res.status(500).json({ error: 'Verify not configured' });
  const schema = z.object({ phone: z.string().min(8), code: z.string().min(4) });
  const parsed = schema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: parsed.error.flatten() });

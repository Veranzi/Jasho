...
creditRouter.post('/link-token', requireAuth, async (req, res) => {
...
creditRouter.post('/public-token', requireAuth, async (req, res) => {
...
creditRouter.get('/score', requireAuth, async (req, res) => {
    const uid = req.user.uid;
    const doc = await firestore.collection('plaid').doc(uid).get();
    if (!doc.exists)
        return res.status(400).json({ error: 'plaid_not_linked' });
...
function buildFinancialFeatures(transactions, accounts) {
...
async function aiCreditScore(features) {
    if (!openai)
        return 600;

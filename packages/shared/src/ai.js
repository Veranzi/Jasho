...
exports.deriveAiInsights = deriveAiInsights;
function computeCreditScore(user, transactions, otherLoansOutstanding = 0) {
    const now = Date.now();
    const income = sumAmounts(transactions.filter((t) => t.type === "deposit"));
    const expenses = sumAmounts(transactions.filter((t) => t.type === "withdrawal" || t.type === "payment"));
    ...
    const normalized = factors.reduce((acc, f) => acc + f.weight * f.value, 0);
...
function deriveAiInsights(user, transactions) {
    const now = Date.now();
    const income = sumAmounts(transactions.filter((t) => t.type === "deposit"));
    const expenses = sumAmounts(transactions.filter((t) => t.type === "withdrawal" || t.type === "payment"));
...
function rollingMonthlyAverage(transactions, types) {
    const now = Date.now();
...
function stabilityScore(transactions) {
...
function suggestBudgets(transactions) {
    const categories = {};
    for (const t of transactions) {
        const category = t.metadata?.["category"] || "misc";
...
function paymentOnTimeRatio(transactions) {
